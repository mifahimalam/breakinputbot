...
def log_to_sheet(sheet, username, display_name, action_time, action_type):
    try:
        ...
        user_rows = [idx for idx, row in enumerate(rows) if row and row[0] == username]
        
        if action_type == "start":
            if user_rows:
                last_user_row = user_rows[-1]
                if len(rows[last_user_row]) < 4 or not rows[last_user_row][3]:  # If end time is empty
            ...
            if user_rows:
                last_user_row = user_rows[-1]
                if len(rows[last_user_row]) >= 3 and rows[last_user_row][2] and (len(rows[last_user_row]) < 4 or not rows[last_user_row][3]):  # If start time exists and end time is empty
        ...
        print(f"Successfully logged {action_type} action for {username} at {action_time}")
